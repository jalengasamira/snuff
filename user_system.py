...
def registrar_partida(usuario, num_partida, stats):
    fecha = stats["date"]
    x = stats.get("score", 0)
    y = stats.get("kills", 0)
    obs = stats.get("character", "")
    with open("partidas.csv", "a", encoding="utf-8") as f:
        f.write(f"{usuario},{num_partida},{fecha},{x},{y},{obs}\n")


def registrar_colision(usuario, num_partida, x, y, obs):
    """
    Ahora 'usuario' es el nombre de usuario (string).
    Se busca el diccionario correspondiente.
    """
    users = load_users()
    user = next((u for u in users if u["usuario"] == usuario), None)
    if not user:
        return  # Si no se encuentra el usuario, no registra
